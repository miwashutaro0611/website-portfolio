...
function outPutRss($rss, $maxsize = 5) {
	...
	foreach ($items as $item) {
		if($i >= $maxsize){
			...
			printf('<dl class="cfx"><dt class="time">'.$time.'</dt><dt class="news_content">'.$title.'</dt></dl>');
			printf('<dd class="text">%s</dd>', $content);
			printf('<dd class="link txR"><a href="'.$link.'" target="_blank">このニュースを見る</a></dd>');
		}
	}
}
;?>
